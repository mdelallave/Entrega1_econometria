...
% 
%     Constant            0             0            NaN             NaN
%     AR{4}        -0.20215       0.06673        -3.0293       0.0024513
%     AR{5}        -0.49087       0.14727        -3.3332      0.00085865
%     AR{10}        0.21741       0.13828         1.5722          0.1159
%     MA{4}         0.17407      0.068262           2.55        0.010772
%     MA{5}         0.41752       0.14688         2.8425       0.0044757
%     MA{10}       -0.28374       0.13335        -2.1278        0.033357
%     MA{12}      -0.068092      0.018238        -3.7335      0.00018885
...
%% Predicciones 
var_resid = estmodelo_imp2.Variance;

M = 1.0e4; % número de trayectorias simuladas
n = 56; % número de observaciones por trayectoria
epsilon = normrnd(0,sqrt(var_resid),M,n + 17); 

% Parameters
phi_4 = estmodelo_imp2.AR{4};
phi_5 = estmodelo_imp2.AR{5};
phi_17= estmodelo_imp2.AR{17};
theta_4 = estmodelo_imp2.AR{4};
theta_5 = estmodelo_imp2.MA{5};
theta_12 = estmodelo_imp2.MA{12};
theta_17 = estmodelo_imp2.MA{17};


f_r = zeros(M,n + 17);

for i = 1:M
    epsilon(i,1:17) = res4(end-16:end);
    f_r(i,1:17) = dlibex(end-16:end); % Primeras diferencias logarítmicas
    for j = 18:n + 17
    f_r(i,j) =  phi_4*f_r(i,j - 4) + ...
                phi_5*f_r(i,j - 5) + phi_17*f_r(i,j - 17) - ... 
                theta_4*epsilon(i,j - 4) - theta_5*epsilon(i,j - 5) - ...
                theta_12*epsilon(i,j - 12) - theta_17*epsilon(i,j - 17) + ...
                epsilon(i,j); 
    end
end
% Graficamos las simulaciones y su media.
figure;
plot(1:n+17,f_r(1:50,:))
hold on
plot(1:n+17,mean(f_r),'k','LineWidth',1.2)
